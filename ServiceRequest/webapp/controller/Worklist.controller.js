...
], function(BaseController, JSONModel, formatter, Filter, FilterOperator, Guid) {
	"use strict";

	return BaseController.extend("zz.ServiceRequest.controller.Worklist", {
		...
		onCreateTicket: function() {
			var oEntity = this._oPopover.getContent()[0].getValue();
			oEntity = JSON.parse(oEntity);
			oEntity.UUID = this._generateGuid();

			if (oEntity) {
				var oDataModel = this.getModel();
				oDataModel.create("/Ticket", oEntity);
		...
		onSearch: function(oEvent) {
			if (oEvent.getParameters().refreshButtonPressed) {
				...
				if (sQuery && sQuery.length > 0) {
					aTableSearchState = [new Filter("Subject", FilterOperator.Contains, sQuery)];
				}
